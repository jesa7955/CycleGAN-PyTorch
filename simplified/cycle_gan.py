...
def save_samples(iteration, fixed_Y, fixed_X, G_YtoX, G_XtoY, opts):
    ...
    reconstructed_X, reconstructed_Y = utils.to_data(reconstructed_X), utils.to_data(reconstructed_Y)
    ...
    path = os.path.join(opts.sample_dir, 'sample-{:06d}-reconstructed-X.png'.format(iteration))
    scipy.misc.imsave(path, merged)
    print('Saved {}'.format(path))

    merged = merge_images(Y, reconstructed_Y, opts)
    path = os.path.join(opts.sample_dir, 'sample-{:06d}-reconstructed-Y.png'.format(iteration))
    scipy.misc.imsave(path, merged)
    print('Saved {}'.format(path))
...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        #adversarial_loss = nn.MSELoss(reduction='elementwise_mean')
        adversarial_loss = nn.MSELoss(size_average=True)
        ...
        D_X_loss = adversarial_loss(D_X(images_X), torch.ones(images_X.shape[0]).to(device))
        D_Y_loss = adversarial_loss(D_Y(images_Y), torch.ones(images_Y.shape[0]).to(device))
        #D_X_loss = torch.sum((D_X(images_X) - 1) ** 2) / images_X.shape[0]
        #D_Y_loss = torch.sum((D_Y(images_Y) - 1) ** 2) / images_Y.shape[0]
        ...
        D_X_loss = adversarial_loss(D_X(fake_X), torch.zeros(fake_X.shape[0]).to(device))
        #D_X_loss = torch.sum(D_X(fake_X) ** 2) / fake_X.shape[0]

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = adversarial_loss(D_Y(fake_Y), torch.zeros(fake_Y.shape[0]).to(device))
        #D_Y_loss = torch.sum(D_Y(fake_Y) ** 2) / fake_Y.shape[0]
        ...
        g_loss = adversarial_loss(D_X(fake_X), torch.ones(fake_X.shape[0]).to(device))
        #g_loss = torch.sum((D_X(fake_X) - 1) ** 2) / fake_X.shape[0]

        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = adversarial_loss(reconstructed_Y, images_Y)
            #cycle_consistency_loss = torch.sum((images_Y - reconstructed_Y) ** 2) / images_Y.shape[0]
        ...
        g_loss = adversarial_loss(D_Y(fake_Y), torch.ones(fake_Y.shape[0]).to(device))
        #g_loss = torch.sum((D_Y(fake_Y) - 1) ** 2) / fake_Y.shape[0]

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = adversarial_loss(reconstructed_X, images_X)
            #cycle_consistency_loss = torch.sum((images_X - reconstructed_X) ** 2) / images_X.shape[0]
            g_loss += cycle_consistency_loss
    ...
    save_samples(opts.train_iters, fixed_Y, fixed_X, G_YtoX, G_XtoY, opts)
